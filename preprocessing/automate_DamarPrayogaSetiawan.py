...
def load_data(source: str) -> pd.DataFrame:
    ...
    Jika URL gagal, mencoba fallback ke file lokal titanic_raw.csv.
    ...
    try:
        ...
        print(f"[load_data] Dataset berhasil dimuat dari: {source}")
    except Exception as e:
        print(f"[load_data] Gagal memuat dari {source}: {e}")
        # Fallback: cari file lokal di folder yang sama dengan script
        script_dir = os.path.dirname(os.path.abspath(__file__))
        local_candidates = [
            os.path.join(script_dir, "titanic_raw.csv"),
            os.path.join(script_dir, "titanic.csv"),
        ]
        loaded = False
        for path in local_candidates:
            if os.path.exists(path):
                print(f"[load_data] Fallback ke file lokal: {path}")
                df = pd.read_csv(path)
                loaded = True
                break
        if not loaded:
            # Last resort: download via requests
            print("[load_data] Mencoba download ulang via requests...")
            import requests, io
            url = "https://raw.githubusercontent.com/datasciencedojo/datasets/master/titanic.csv"
            r = requests.get(url, timeout=30)
            r.raise_for_status()
            df = pd.read_csv(io.StringIO(r.text))
            print(f"[load_data] Download berhasil via requests.")

    print(f"[load_data] Dataset → {df.shape[0]} baris, {df.shape[1]} kolom")
